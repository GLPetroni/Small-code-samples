...
class HashMap:
    ...
    def resize_table(self, capacity):
        ...
        if capacity <= 0 or capacity == self.capacity:
            return
        for i in range(capacity):
            tmp.append(LinkedList())
        for i in range(self.capacity):
            while self.buckets[i].head is not None:
                k = self.buckets[i].head.key
                _key = self.hash_function(k)
                index = _key % tmp.length()
                if _key is not None:
                    val = self.buckets[i].contains(k)
                    if val.value is not None:
                        tmp[index].insert(k, val.value)
                        self.buckets[i].remove(k)
        self.buckets = tmp
        self.capacity = tmp.length()
